...
describe('runFormatPhase', () => {
  it('should return undefined for unformatted clean file (no oxfmt or no changes)', () => {
    ...
    const result = runFormatPhase('/tmp/nonexistent.ts', log as unknown as ReturnType<typeof makeMockLogger>)
  ...
  it('should not throw on any input', () => {
    const log = makeMockLogger()
    expect(() => runFormatPhase('/tmp/any-file.ts', log as unknown as ReturnType<typeof makeMockLogger>)).not.toThrow()
  })
})
